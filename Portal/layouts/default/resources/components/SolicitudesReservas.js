...
function SolicitudesReservas_IndexView_Component($scope, $api, $webapp, $modal, sharedModalService, $translatePartialLoader, $rootScope, $http, $translate) {
	...
	$scope.create = function () {
		//jQuery(document).ready(function(){
			setTimeout(function(){
				jQuery( "#solicitudesReservasForm" )
				    .parent().parent().addClass( "editModal" );
				    }, 50);
		...
		//document.getElementById('solicitudesReservasForm').style.backgroundColor = 'red';
	}
		...
		$api.get($scope.module + '/FetchRecords', {
			...
			language: language
				...
				.success(function (result) {
					...
					if(lang == 'en_us'){
						$('a[data-columnname="Contacto"]').html("Contact");
						$('a[data-columnname="Estado"]').html("State");
						$('a[data-columnname="Fecha de Comienzo"]').html("Start Date");
						$('a[data-columnname="Fecha de Fin"]').html("End Date");
						$('a[data-columnname="Días"]').html("Days");
						$('a[data-columnname="Correo electrónico"]').html("E-mail");
						$('a[data-columnname="Número de Confirmación"]').html("Confirm Number");
						$('a[data-columnname="Cantidad de Pasajeros"]').html("Passengers Quantity");

						$('span[title="Confirmada"]').html("Confirmed");
						$('span[title="Pendiente"]').html("Pending");

						for(let i = 0; i < $scope.records.length; i++){
							if($scope.records[i].Estado == "Confirmada")
								$scope.records[i].Estado = "Confirmed";
							else if($scope.records[i].Estado == "Pendiente")
								$scope.records[i].Estado = "Pending";
						}
					}
					else if(lang == 'es_es'){
						for(let i = 0; i < $scope.records.length; i++){
							if($scope.records[i].Estado == "Confirmated")
								$scope.records[i].Estado = "Confirmada";
						}
					}
					$webapp.busy(false);
				});
...
function SolicitudesReservas_DetailView_Component($scope, $api, $webapp, $translatePartialLoader, sharedModalService, $modal, $http) {
	...
	//jQuery(document).ready(function(){
		setTimeout(function(){
			jQuery( "#editSolicitudReservaHeader" )
...
function SolicitudesReservas_EditView_Component($scope, $modalInstance, record, api, webapp, module, $timeout, $translatePartialLoader, language, $filter, $http, editStatus) {
	...
	//console.log("----->" + JSON.stringify(jQuery('#solicitudesReservasForm').parent()));

	/*document.getElementById('solicitudesReservasForm').style.backgroundColor = 'red';

	jQuery('#solicitudesReservasForm').closest('.modal-dialog').css('width', '1000px');*/
	...
	$scope.save = function (editar) {
		...
		if(editar == true)
			params.recordId = "42x" + $scope.idSolicitud;
