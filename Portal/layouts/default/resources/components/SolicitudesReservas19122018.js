...
function SolicitudesReservas_EditView_Component($scope, $modalInstance, record, api, webapp, module, $timeout, $translatePartialLoader, language, $filter, $http, editStatus) {
	...
	.success(function (response) {
		...
		if(url.indexOf("&id=") < 0) $scope.hotel = $scope.hoteles[0];
	});                  
	...
	.success(function (response) {
		...
		if(url.indexOf("&id=") < 0) $scope.rescontacto = $scope.contactos[0];
	});*/

	$scope.estados = [{id : 'Pendiente', name : 'Pendiente'}, {id : 'Confirmada', name : 'Confirmada'}];
	if(url.indexOf("&id=") < 0) $scope.resestado = {id : 'Pendiente', name : 'Pendiente'};
	...
	if(url.indexOf("&id=") > -1){
		...
		$http.get('index.php?module=SolicitudesReservas&api=getDataSolicitudReserva', {
			...
			$scope.hotel = {id : $scope.solicitud['hotel'], name : $scope.solicitud['hotel']};
