...
$(document).ready(function(){
	$(document).off('click', '#descargarRelacionados');
	$(document).on('click','#descargarRelacionados',function(){
		ExportarExcelPHP();
	})

	/*
	Esta funcion trae las campos que estan en la cabecera
	devuelve fields que es el nombre del campo de la base de datos (me sirve para la consulta)
	y devuleve campos que es el nombre traducido (me sirve para las columnas del excel)

	*/
	function getHeaders(){
		var fields='';
		var campos='';
		var columnas=$(".listViewHeaders").children();
		for(i=2; i < columnas.length -1 ; i++){
			fields=fields+$(columnas[i]).find('a').attr('data-fieldname')+",";
			campos = campos+$(columnas[i]).find('a').text()+",";
		}
	...
	function ExportarExcelPHP() {
  			...
  			var campos = getHeaders();
  			var thisInstance = this;
  			var relacionado = $(".nav.nav-tabs").find('li.tab-item.active').attr('data-label-key');
	        var params = {
	            'module': 	"Campaigns",
	            'action': 	"exportarExcelPHP",
	            'id':  		$("#recordId").val(),
	            'fields' : campos[0],
	            'campos': campos[1],
	            'relacionado' : $(".nav.nav-tabs").find('li.tab-item.active').attr('data-label-key')
	        }
	        AppConnector.request(params).then(
	                function (data) {
	                	...
	                	if (relacionado=='Contacts') {
	                		var to= '/'+(window.location.pathname.split('/')[1])+'/excelContactosCampania.php';
							to = to.replace("/index.php", "");
	                		
	                		location.href=to;
	                	}
	                	if (relacionado=='Accounts') {
	                		var to= '/'+(window.location.pathname.split('/')[1])+'/excelCuentasCampania.php';
							to = to.replace("/index.php", "");
	...
	function ExportarExcelJavaScript(){
	    var tab_text="<table border='2px'><tr bgcolor='#87AFC6'>";
	    var textRange; var j=0;
	    tab = document.getElementById('listview-table'); // id of table

	    for(j = 0 ; j < tab.rows.length ; j++) 
	    {   
			if(j==1)continue;
			for(i=2;i<tab.rows[j].cells.length; i++){
