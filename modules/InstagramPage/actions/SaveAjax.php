...
class InstagramPage_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	*/
	public function process(Vtiger_Request $request) {
		global $log;
		$log->debug("EN LA FUNCION SAVEAJAX InstagramPage");
		...
		$rs 		= $adb->pquery('SELECT InstagramPageid FROM vtiger_InstagramPage WHERE fbpage_id = ?', array($page_id) );
	$log->debug("aca");
		if( $adb->num_rows($rs) == 0 ){ //Si no existe la creo
			...
			if($longLifeToken) $recordModel->set('fbpaccess_token',$longLifeToken);
			$log->debug("aca");
			$recordModel->save();
			$log->debug("aca");
