...
//include_once './lp_webform_capture.php';
...
class Webform_Capture {
	...
	function captureNow($request) {
		...
		try {
			...
			/*echo $parameters['top_selling_destinations'];
			exit();*/
			if(substr_count($parameters['top_selling_destinations'], "|##|") != 1){
				throw new Exception("Top 2 selling destination has not two items selected");
				return;
			}

			if(substr_count($parameters['top_specialities'], "|##|") != 1){
				throw new Exception("Top 2 specialities has not two items selected");
				return;
			}

			$sql = "SELECT contactid FROM vtiger_contactdetails INNER JOIN vtiger_crmentity ON contactid = crmid WHERE email = ? AND deleted = 0";
			$result = $adb->pquery($sql, array($parameters['email']));
			if($adb->num_rows($result) > 0){
				throw new Exception("The e-mail is in use");
				return;
			}
