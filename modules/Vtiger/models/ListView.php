...
class Vtiger_ListView_Model extends Vtiger_Base_Model {
	...
	public function getListViewLinks($linkParams) {
		...
		global $log;
		$log->debug("ACA ESTAN LOS LINKS:::");
		$log->debug(json_encode($links['LISTVIEWBASIC']));
     ...
     * @return <Array> - Associative array of record id mapped to Vtiger_Record_Model instance.
     */
    public function getListViewEntriesFilter($pagingModel,$searchParams=NULL) {
        $db = PearDatabase::getInstance();
        ...
        $queryGenerator = $this->get('query_generator');
        $listViewContoller = $this->get('listview_controller');
        //BF 08/06/2016 agrego        
        if(empty($searchParams)) {
            $searchParams = array();
        }else{
			//Le pone un array de más
        ...
        $listViewRecordModels = array();
        $listViewEntries = $listViewContoller->getListViewRecordsFilter($moduleFocus, $moduleName, $listResult);
