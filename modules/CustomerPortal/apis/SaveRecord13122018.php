...
class CustomerPortal_SaveRecord extends CustomerPortal_FetchRecord {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			if (in_array($module, array('HelpDesk', 'Documents', 'Assets', 'Quotes', 'Contacts', 'Accounts', 'SolicitudesReservas'))) {
				$recordId = $request->get('recordId');
				if (!empty($recordId)) {
				...
				$log->debug("Values mauro: " . var_export($valuesJSONString, true));
				$values = "";
				...
				if ($module !== 'Contacts' && $module !== 'Accounts' && $module !== "SolicitudesReservas") {
				...
				try {
					if (vtws_recordExists($recordId)) {
						...
						if (!empty($values)) {
							...
							$log->debug("mauro value: " . $name . " => " . $value);
