		...
		if ($current_user) {
			...
			foreach ($activeModules as $module) {
				...
				if ($module == 'Faq') {
					...
					$queryResult = vtws_query("SELECT id, resestado, resfechacom, resfechafin, resdias, hotel, rescreatedtime FROM SolicitudesReservas " . $filterClause, $current_user);
					$log->debug("en el modulo mauro 2");					
				} else if ($module == 'RedimirDias') {
					$log->debug("en el modulo mauro ");
					$filterClause = sprintf("ORDER BY redcreatedtime DESC LIMIT %s;", $limit);
					$log->debug("en el modulo mauro 1");
					$queryResult = vtws_query("SELECT id, redestado, redfechacom, redfechafin, reddias, hotel, redcreatedtime FROM RedimirDias " . $filterClause, $current_user);
					$log->debug("en el modulo mauro 2");					
				} else {
					$queryResult = vtws_query_related($sql, $parentId, CustomerPortal_Utils::getRelatedModuleLabel($module), $current_user, $filterClause);
				}
				...
				if (!empty($queryResult)) {
					...
					for ($i = 0; $i < $num_rows; $i++) {
						...
						if ($module == 'SolicitudesReservas') {
							...
							//$record['rescreatedtime'] = date('d/m/Y H:i:s', strtotime($queryResult[$i]['rescreatedtime']));
						...
						if ($module == 'RedimirDias') {
							...
							$record['hotel'] = $queryResult[$i]['hotel'];
							//$record['redcreatedtime'] = date('d/m/Y H:i:s', strtotime($queryResult[$i]['redcreatedtime']));
							//$record['fecha'] = date('d-m-Y', strtotime($queryResult[$i]['createdtime']));
