...
class CustomerPortal_FetchRecords extends CustomerPortal_API_Abstract {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			$log->debug("active fields mauro: " . var_export($activeFields, true));

			if($module == "SolicitudesReservas"){
				$activeFields = array(0 => 'resestado',
									  1 => 'resfechacom',
									  2 => 'resfechafin',
									  3 => 'resdias',
									  4 => 'rescorreo',
				...
				$activeFields = array(0 => 'redestado',
									  1 => 'redfechacom',
									  2 => 'redfechafin',
									  3 => 'reddias',
									  4 => 'redcorreo',
				...
				$activeFields = array(0 => 'notresumen',
									  1 => 'notnoticia',
									  2 => 'notdesde',
									  3 => 'nothasta',
									  4 => 'notidioma');
			}
			/*$activeFields = array(0 => 'rescontacto',
								  1 => 'resestado',
								  3 => 'resfechafin',
								  4 => 'resdias',
								  5 => 'rescorreo',
			...
			if ($module == 'Faq') {
				...
				$log->debug("llego hasta aca mauro");
				if ($mode == 'mine') {

					$log->debug("modo mine " . $contactWebserviceId);
					$relatedId = $contactWebserviceId;
					$countResult = vtws_query_related($countSql, $relatedId, $moduleLabel, $current_user);
					$log->debug("en el modo mine mauro por hacer var_dump");
					$log->debug(var_export($countResult, true));
					$count = $countResult[0]['count'];

					//$limitClause = sprintf('ORDER BY %s %s LIMIT %s,%s', $orderBy, $order, ($page * $pageLimit), $pageLimit);
					//$limitClause = sprintf('LIMIT %s,%s;', ($page * $pageLimit), $pageLimit);
					$result = vtws_query_related($sql, $relatedId, $moduleLabel, $current_user);
				} else if ($mode == 'all') {
					$log->debug("modo all: " . $module);
					if (in_array($module, array('Products', 'Services', 'SolicitudesReservas', 'Noticias'))) {
						$log->debug("modo all en el primero if");
						$countSql = sprintf('SELECT count(*) FROM %s;', $module);
						$sql = sprintf('SELECT %s FROM %s', $fields, $module);
						$limitClause = sprintf('LIMIT %s,%s;', ($page * $pageLimit), $pageLimit);
						if($module == "Noticias")
							$limitClause = sprintf('ORDER BY notorden ASC LIMIT %s,%s;', ($page * $pageLimit), $pageLimit);
						$sql = $sql.' '.$limitClause;
						$log->debug("sql mauro: " . $sql);
						$result = vtws_query($sql, $current_user);
						$countResult = vtws_query($countSql, $current_user);
						$count = $countResult[0]['count'];
					} else {
