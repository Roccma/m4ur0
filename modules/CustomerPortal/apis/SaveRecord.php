...
class CustomerPortal_SaveRecord extends CustomerPortal_FetchRecord {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			if (in_array($module, array('HelpDesk', 'Documents', 'Assets', 'Quotes', 'Contacts', 'Accounts', 'SolicitudesReservas', 'RedimirDias'))) {
				$recordId = $request->get('recordId');
				if (!empty($recordId)) {
				...
				$log->debug("Values mauro: " . $valuesJSONString);
				$values = "";
				...
				if ($module !== 'Contacts' && $module !== 'Accounts' && $module !== "SolicitudesReservas" && $module != "RedimirDias") {
				...
				try {
					if (vtws_recordExists($recordId)) {
						...
						if ($module == 'SolicitudesReservas') {
							...
							$this->recordValues['rescontacto'] = $contactId;
						...
						if ($module == 'RedimirDias') {
							$contactId = vtws_getWebserviceEntityId('Contacts', $this->getActiveCustomer()->id);
							$this->recordValues['redcontacto'] = $contactId;
							$this->recordValues['from_portal'] = true;
							$accountId = $this->getParent($contactId);
							if (!empty($accountId))
								$this->recordValues['parent_id'] = $accountId;
